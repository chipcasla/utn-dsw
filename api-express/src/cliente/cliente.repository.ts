...
export class ClienteRepository implements Repository<Cliente> {
  ...
  public findOne(item: { id: string }): Cliente | undefined {
    return clientes.find((cliente) => cliente.id && cliente.id.toString() === item.id)
  }
  ...
  public update(item: Cliente): Cliente | undefined {
    const clienteIdx = clientes.findIndex((cliente) => item.id && cliente.id && cliente.id.toString() === item.id.toString())
  ...
  public delete(item: { id: string }): Cliente | undefined {
    const clienteIdx = clientes.findIndex((cliente) => cliente.id && cliente.id.toString() === item.id)

    if (clienteIdx !== -1) {
      const deletedCharacters = clientes[clienteIdx]
